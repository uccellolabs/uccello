...
trait RelatedlistTrait
{
    ...
    public function getDependentListCount(Relatedlist $relatedList, int $recordId) : int
    {
        ...
        return $relatedModel::where(function (Builder $_query) use ($recordId, $relatedList, $relatedField) {
            ...
            // Is column is not int, check by uuid (else it compares only with the first number of the uuid string)
            $relatedModelClass = $relatedList->module->model_class;
            $columnTypeResult = DB::select(DB::raw("SHOW COLUMNS FROM `".(new $relatedModelClass)->getTable()."` WHERE Field = '".(new $relatedModelClass)->getKeyName()."'"));
            if (!empty($record->uuid) && !empty($columnTypeResult[0]) && strpos($columnTypeResult[0]->Type, 'int') === false) {
                $_query->orWhere($relatedField->column, $record->uuid);
            }
        })
        ->count();
