...
@section('sidebar-main-menu')
    <ul class="list">
        ...
        <?php $homeModule = ucmodule('home'); ?> @if (isset($domain) && $homeModule->isActiveOnDomain($domain) && Auth::user()->canRetrieve($domain, $homeModule))
        {{-- Home module --}}
        ...
        @endif {{-- All modules except Home --}} @if (isset($modules) && isset($domain) && isset($module)) @foreach ($modules as
        $_module) @continue ($_module->name === 'home' || !$_module->isActiveOnDomain($domain) || !Auth::user()->canRetrieve($domain,
        $_module) || (!$admin_env && $_module->isAdminModule()) || ($admin_env && !$_module->isAdminModule()))
        ...
        @endforeach @endif @yield('sidebar-main-menu-after') {{-- Display admin menu for admin users --}} @if (Auth::user()->is_admin
        && !$admin_env)
        ...
        @yield('sidebar-admin-menu-after') @endif
    </ul>
    @show @yield('sidebar-menu-after')
</div>